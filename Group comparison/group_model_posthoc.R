...
summary(pred_rt$RT)
write_csv(pred_rt, "Group comparison/posterior_by_group.csv")


pred_std <- pred %>% as_tibble() %>%
  mutate(row = 1:n(),
         sigma = sigma) %>%
  gather(key = COND, value = RT, -row,-sigma) %>%
  separate(COND, sep = "_", into = c("Group", "Dependency", "Block")) %>%
  dplyr::select(-row) %>%
  pivot_wider(names_from = Block, values_from = RT) %>%
  mutate(value = `7`- `6`,
         std_value = value / sigma) %>%
  dplyr::select(-sigma, -`6`, -`7`, -value)
...
write_csv(pred_rt_es, "Group comparison/posterior_by_group_effectsize.csv")
...
write_csv(pred_rt_es, "Group comparison/posterior_by_group_effectsize_dependency.csv")
