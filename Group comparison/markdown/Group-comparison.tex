...
\midrule
Main effects &  &  & \\
\ \ \ Age Group & -391 & [-455, -330] & >.999\\
\ \ \ Dependency & 8 & [-29, 48] & .316\\
\ \ \ Adjacency & -9 & [-37, 21] & .714\\
\ \ \ Block & -68 & [-104, -33] & >.999\\
Interactions &  &  & \\
\ \ \ Dependency $\times$ Block & -36 & [-75, 10] & .933\\
\ \ \ Adjacency $\times$ Block & -11 & [-42, 21] & .765\\
\ \ \ Age $\times$ Block & -82 & [-112, -50] & >.999\\
\ \ \ Age $\times$ Dependency & -66 & [-95, -34] & >.999\\
\ \ \ Age $\times$ Adjacency & -13 & [-41, 12] & .86\\
\ \ \ Dependency $\times$ Block $\times$ Age & -39 & [-73, -8] & .99\\
\ \ \ Adjacency $\times$ Block $\times$ Age & 68 & [34, 108] & <.001\\
Variance components &  &  & \\
\ \ \ $\sigma_{adults}$ & 0.28 & [0.27, 0.29] & --\\
...
First, we observed an interaction of Dependency (levels: dependency, baseline), Block, and Age Group. For the adult group, we observed a small effect for a slowdown from Block 6 to 7 in the dependency condition (\(\hat{\delta}\) = 0.26, 95\% HPDI{[}0.12, 0.43{]}, ROPE = 0\%) but negligible evidence for a slowdown in baseline trials (\(\hat{\delta}\) = 0, 95\% HPDI{[}-0.11, 0.08{]}, ROPE = 100\%). For the children group, we observed negligible to small slowdown effects in both the dependency condition (\(\hat{\delta}\) = 0.11, 95\% HPDI{[}-0.02, 0.27{]}, ROPE = 35\%) and for the baseline condition (\(\hat{\delta}\) = 0.15, 95\% HPDI{[}0.04, 0.25{]}, ROPE = 14\%).

Second, we observed an interaction of Adjacency (levels: adjacent, nonadjacent), Block, and Age Group. Slowdown effects were found in adults for both dependencies with a negligible to small effect for the adjacent condition (\(\hat{\delta}\) = 0.23, 95\% HPDI{[}0.01, 0.46{]}, ROPE = 8\%) and a small to medium strong effect for nonadjacent dependencies (\(\hat{\delta}\) = 0.30, 95\% HPDI{[}0.12, 0.52{]}, ROPE = 0\%). For the children group we observed a negligible to small slowdown effect for both adjacent dependencies (\(\hat{\delta}\) = 0.08, 95\% HPDI{[}-0.11, 0.30{]}, ROPE = 55\%) and for nonadjacent dependencies (\(\hat{\delta}\) = 0.16, 95\% HPDI{[}-0.04, 0.36{]}, ROPE = 25\%).
