...
samps %<>% filter(grepl("alpha|beta", Parameter)) %>%
...
pred_std <- pred %>% as_tibble() %>%
  ...
  dplyr::select(-starts_with("sigma"), -`7`, -`6`, -`1`, -diff_transfer, -diff_learning)
summary(pred_std)
...
pred_std <- pred %>% as_tibble() %>%
  ...
  dplyr::select(-starts_with("sigma"), -`7`, -`6`, -`1`, -transfer, -learning)
